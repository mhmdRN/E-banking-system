...
public class WithdrawController extends ErrorHandler implements Initializable{
	@FXML
	...
	@FXML
	void withdraw(ActionEvent event) throws SQLException {
		amount.setStyle(null);
		datepicker.setStyle(null);
		accountNames.setStyle(null);
		if(amount.getText().isEmpty() || (datepicker.getValue()==null) || accountNames.getSelectionModel().getSelectedItem()==null) {
		...
		if(f.addTransaction(LoginController.currentEmployer, cusId,accName, "WITHDRAW", date,Integer.parseInt(amount.getText()),accId)) {
		...
		DEMO.notify.register(new Customer(customerId,f.getCustomerEmailByid(customerId),Integer.parseInt(amount.getText()),"Withdraw"));
	}
